...
const NestThermostat = function (nest, deviceId, deviceInfo) {
  ...
  this.setMode = function (strMode) {
    ...
    return new Promise((resolve, reject) => {
      ...
      nest
        .command(deviceId, CMD_MODE, params)
        .then((res) => {
  ...
  this.setHeatTemperature = function (temp) {
    ...
    return new Promise((resolve, reject) => {
      if (self.getMode() != "HEAT") {
        const err =
          "setHeatTemperature: thermostat not in HEAT mode" + self.getMode();
        console.err(err);
        reject(err);
        return;
      }
      ...
      nest
        .command(deviceId, CMD_HEAT_TEMP, params)
        .then((res) => {
          const result = res.data;
  ...
  this.setCoolTemperature = function (temp) {
    ...
    return new Promise((resolve, reject) => {
      if (self.getMode() != "COOL") {
        const err =
          "setCoolTemperature: thermostat not in COOL mode" + self.getMode();
        console.err(err);
        reject(err);
        return;
      }
      ...
      nest
        .command(deviceId, CMD_COOL_TEMP, params)
        .then((res) => {
          const result = res.data;
  ...
  this.setEcoMode = function (on) {
    ...
    return new Promise((resolve, reject) => {
      ...
      nest
        .command(deviceId, CMD_ECO, params)
        .then((res) => {
          const result = res.data;
